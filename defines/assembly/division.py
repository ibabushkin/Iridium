...
if __name__ == '__main__':
    ...
    if not f.interactive:
        d = DivisionParser(l)
        if f.next_imul_threshold:
            d.next_imul = int(f.next_imul_threshold)
        if f.next_sar_threshold:
            d.next_sar = int(f.next_sar_threshold)
        d.find_interestig_code_sequences()
    else:
        d = DivisionParser(l)
        magic = raw_input(
            'Enter constant as hex number prefixed by 0x (input not verified!): ')
        rshift = int(
            raw_input('Enter rshift amount (must be int, input not verified): '))
